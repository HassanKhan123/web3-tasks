...
web3.eth.getTransactionReceipt(txnHash, function (e, data) {
  if (e !== null) {
    ...
    if (!data.status) {
      console.log(
        "The contract execution was not successful, check your transaction !"
      );
    } else {
      console.log(
        "Execution worked fine! And the gas used in this txn is :",
        web3.utils.fromWei(
          (
            data.gasUsed * web3.utils.hexToNumber(data.effectiveGasPrice)
          ).toString()
        )
      );
    }
